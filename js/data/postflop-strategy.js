...
//
// 12 spot categories x 8 board textures x 10 hand strengths = 960 entries
...
(function() {
  ...
  var HAND_STRENGTHS = [
    'air', 'weak_draw', 'gutshot', 'oesd_or_fd', 'weak_pair',
    'second_pair', 'top_pair_weak', 'top_pair_strong', 'overpair', 'two_pair_plus'
  ];
  ...
  // These represent a "neutral" board texture (dry_rainbow for betting spots).
  // Board texture modifiers are applied on top.
  // -------------------------------------------------------------------------
  ...
  var base_IP_cbet_flop = {
    ...
    two_pair_plus:    { check: 0.25, bet_33: 0.20, bet_67: 0.25, bet_100: 0.30 }
  };
  ...
  var base_OOP_cbet_flop = {
    ...
    two_pair_plus:    { check: 0.30, bet_33: 0.15, bet_67: 0.25, bet_100: 0.30 }
  };
  ...
  var base_IP_turn_barrel = {
    ...
    two_pair_plus:    { check: 0.15, bet_33: 0.10, bet_67: 0.30, bet_100: 0.45 }
  };
  ...
  var base_OOP_turn_barrel = {
    ...
    two_pair_plus:    { check: 0.20, bet_33: 0.08, bet_67: 0.27, bet_100: 0.45 }
  };
  ...
  var base_IP_river_bet = {
    ...
    two_pair_plus:    { check: 0.10, bet_33: 0.05, bet_67: 0.25, bet_100: 0.60 }
  };

  // OOP river bet: most polarized OOP, even more checking with medium hands
  var base_OOP_river_bet = {
    ...
    two_pair_plus:    { check: 0.12, bet_33: 0.05, bet_67: 0.23, bet_100: 0.60 }
  };

  // ----- FACING BET SPOTS (actions: fold, call, raise) -----

  // IP facing c-bet: draws call/raise, air mostly folds, strong hands trap
  var base_IP_facing_cbet = {
    ...
    two_pair_plus:    { fold: 0.01, call: 0.44, raise: 0.55 }
  };

  // OOP facing c-bet: slightly more folding due to positional disadvantage
  var base_OOP_facing_cbet = {
    ...
    two_pair_plus:    { fold: 0.01, call: 0.39, raise: 0.60 }
  };

  // IP facing turn bet: tighter defense, more polarized raising
  ...
  // OOP facing turn bet: harder to defend OOP
  var base_OOP_facing_turn_bet = {
    ...
    two_pair_plus:    { fold: 0.01, call: 0.34, raise: 0.65 }
  };

  // IP facing river bet: very polarized decisions
  var base_IP_facing_river_bet = {
    ...
    two_pair_plus:    { fold: 0.02, call: 0.33, raise: 0.65 }
  };

  // OOP facing river bet: hardest spot
  var base_OOP_facing_river_bet = {
    ...
    two_pair_plus:    { fold: 0.02, call: 0.28, raise: 0.70 }
  };

  // -------------------------------------------------------------------------
  // Board texture modifiers
  // Each modifier adjusts the base frequencies to reflect texture-specific
  // strategic differences. Separate modifiers for betting and facing-bet spots.
  // -------------------------------------------------------------------------

  // For BETTING spots the modifiers shift weight between check and bet sizings.
  // Format: { checkAdj, bet33Adj, bet67Adj, bet100Adj }
  //   - positive checkAdj  => more checking
  //   - negative checkAdj  => less checking (more betting)
  //   - sizing shifts move weight between small/medium/large bets

  var betTextureModifiers = {
    // dry_rainbow is the baseline, no adjustments
    dry_rainbow:       { checkAdj: 0,     bet33Adj: 0,     bet67Adj: 0,     bet100Adj: 0    },
    // two-tone: slightly more checking, larger sizing when betting
    dry_twotone:       { checkAdj: 0.05,  bet33Adj: -0.05, bet67Adj: 0.03,  bet100Adj: 0.02 },
    // wet rainbow: connected board, moderate check increase, larger sizing
    wet_rainbow:       { checkAdj: 0.08,  bet33Adj: -0.06, bet67Adj: 0.02,  bet100Adj: 0.02 },
    // wet two-tone: much more checking, when betting use bigger sizes
    wet_twotone:       { checkAdj: 0.15,  bet33Adj: -0.10, bet67Adj: 0.02,  bet100Adj: 0.03 },
    // monotone: substantially more checking, very little small betting
    monotone:          { checkAdj: 0.25,  bet33Adj: -0.18, bet67Adj: -0.02, bet100Adj: 0.05 },
    // paired dry: higher c-bet, small sizing (range advantage)
    paired_dry:        { checkAdj: -0.08, bet33Adj: 0.10,  bet67Adj: -0.02, bet100Adj: -0.03},
    // paired wet: slightly higher c-bet, small sizing
    paired_wet:        { checkAdj: -0.03, bet33Adj: 0.06,  bet67Adj: 0.00,  bet100Adj: -0.01},
    // highly connected: more checking, bigger sizing when betting
    highly_connected:  { checkAdj: 0.12,  bet33Adj: -0.10, bet67Adj: 0.03,  bet100Adj: 0.03 }
  };

  // For FACING BET spots the modifiers shift fold/call/raise frequencies.
  // Format: { foldAdj, callAdj, raiseAdj }
  ...
  // Hand-strength-specific texture overrides:
  // On monotone boards, hands without a flush draw check/fold much more.
  // On wet boards, draws are more prominent in the betting/calling range.
  // These are applied as additional multipliers after the base modifier.

  // For betting spots: per-hand-strength extra adjustments on specific textures
  var betHandTextureOverrides = {
    monotone: {
      // air without flush draw checks much more
      ...
      // flush draws bet more aggressively on monotone
      oesd_or_fd:     { checkAdj: -0.10, bet33Adj: -0.05, bet67Adj: 0.05,  bet100Adj: 0.10  },
      // strong made hands bet less (vulnerable to flush)
    ...
    wet_twotone: {
      // draws bet/semi-bluff more on wet boards
    ...
    paired_dry: {
      // on paired boards, can bluff more since opponent often missed
      air:            { checkAdj: -0.05, bet33Adj: 0.05,  bet67Adj: 0.00,  bet100Adj: 0.00  },
      // strong hands can bet bigger for value
      two_pair_plus:  { checkAdj: -0.05, bet33Adj: -0.05, bet67Adj: 0.05,  bet100Adj: 0.05  }
    }
  };

  // For facing-bet spots: per-hand-strength extra adjustments on specific textures
  var facingHandTextureOverrides = {
    monotone: {
      // without flush, fold much more
      ...
      // flush draws defend well
      oesd_or_fd:       { foldAdj: -0.05, callAdj: 0.00,  raiseAdj: 0.05  },
      // sets/flushes raise more
      two_pair_plus:    { foldAdj: -0.01, callAdj: -0.05, raiseAdj: 0.06  }
    },
    wet_twotone: {
    ...
    paired_dry: {
      // opponent c-bets more so we defend a bit wider
  ...
  GTO.Data.lookupPostflop = function(spotType, boardTexture, handStrength) {
    var spot = this.PostflopStrategy[spotType];
    if (!spot) return null;
    var texture = spot[boardTexture];
    if (!texture) return null;
    return texture[handStrength] || null;
  };

  // -------------------------------------------------------------------------
  // Metadata for consumers (UI labels, iteration, etc.)
  // -------------------------------------------------------------------------
  ...
  GTO.Data.HandStrengthLabels = {
    ...
    overpair:         'Overpair',
    two_pair_plus:    'Two Pair or Better'
  };
